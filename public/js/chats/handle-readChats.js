...
async function fetchAndLoadChats(receiverId, userToken){
 console.log(receiverId,userToken)
    ...
    if(res.status === 200 && res.data.success){
        ...
        localStorage.setItem('chatBuddyName', chatBuddy)
...
function loadInitialChats(oneToOnChats){
 ...
 if(!Array.isArray(oneToOnChats)) return console.log('no array chats')
...
function loadChatPageHead(){
    const buddy = localStorage.getItem('chatBuddyName')
     console.log(buddy)
     const head = createElement('div','head')
    head.innerHTML  = `
            <div class="left"> 
                <h2>${buddy}</h2> 
                <span class="top-user-typing"></span> 
            </div> 
            <div class="right"> 
                <ul> 
