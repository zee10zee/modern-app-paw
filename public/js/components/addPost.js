...
// const previewContainer = filePreviewContainer.querySelector('')
   console.log('media input ', filePreviewContainer)
    ...
    if(newPostCancelBtn || closeNewPostIcon){
       e.preventDefault() 
       removePreviewMedia(previewContainer)
...
<div class="postHeader">
  ...
  <div class="postHeaderContent">
    ...
    <span class="postDate">${getTimeAgo(newPost.created_at)}</span>
  </div>
</div>
...
if(newPostAlert && postAlert){
    ...
    const picLogo = newMemoryContainer.querySelector('.pic-logo')
    if(!picLogo) console.log('no poic logo')
    
    const mediaInput = newMemoryContainer.querySelector('#mediaFile')
    if(!mediaInput) console.log('no media input')

    
    picLogo.addEventListener('click', (e)=>{
      mediaInput.click()
      console.log('pic logo clicked')
    })
